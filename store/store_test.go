...
func TestMain(m *testing.M) {
	...
	block = state.MakeBlock(state.LastBlockHeight+1, test.MakeNTxs(state.LastBlockHeight+1, 10), nil, nil, new(types.Commit), nil, state.Validators.GetProposer().Address) // TODO
...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	// save a block
	block := state.MakeBlock(bs.Height()+1, nil, nil, nil, new(types.Commit), nil, state.Validators.GetProposer().Address) // TODO
...
func TestLoadBaseMeta(t *testing.T) {
	...
	for h := int64(1); h <= 10; h++ {
		block := state.MakeBlock(h, test.MakeNTxs(h, 10), nil, nil, new(types.Commit), nil, state.Validators.GetProposer().Address) // TODO
		partSet, err := block.MakePartSet(2)
		require.NoError(t, err)
...
func TestLoadBlockMetaByHash(t *testing.T) {
	...
	b1 := state.MakeBlock(state.LastBlockHeight+1, test.MakeNTxs(state.LastBlockHeight+1, 10), nil, nil, new(types.Commit), nil, state.Validators.GetProposer().Address) // TODO
